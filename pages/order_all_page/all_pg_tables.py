...
class OrdersAllPageTables(BasePage):
    ...
    def __init__(self, driver):
        ...
        self.all_pg_tables_const = OrdersAllTablesConst
        from constants.orders_all_page.all_pg_list import OrdersAllListConst
        self.all_pg_list_const = OrdersAllListConst
    ...
    def verify_sum_in_eur_currency_totals(self):
        all_totals_sum_in_eur = self.wait_until_all_displayed(xpath=self.all_pg_tables_const.TOTAL_IN_EUR_TABLE_VALUES_XPATH)
        one_total_sum_in_eur = [Decimal(total_in_eur.text) for total_in_eur in all_totals_sum_in_eur]
        total_sum_in_eur = self.get_element_text(self.all_pg_tables_const.TOTAL_IN_EUR_ALL_XPATH)
    ...
    def verify_sum_price_list_usdt(self):
        total_price_usdt = self.get_element_text(xpath=self.all_pg_tables_const.TOTAL_IN_CURRENCY_USDT_XPATH)
        if self.is_exist(xpath=self.all_pg_list_const.ORDERS_PRICE_USDT_XPATH):
            all_orders_price_usdt = self.wait_until_all_displayed(xpath=self.all_pg_list_const.ORDERS_PRICE_USDT_XPATH)
    ...
    def verify_sum_all_totals_for_teams(self):
        all_totals_sum_in_eur = self.wait_until_all_displayed(xpath=self.all_pg_tables_const.TOTAL_ALL_TEAMS_VALUES_XPATH)
        one_total_sum_in_eur = [Decimal(total_in_eur.text) for total_in_eur in all_totals_sum_in_eur]
        total_team_sum_in_eur = self.get_element_text(self.all_pg_tables_const.TOTAL_ALL_TEAMS_SUM_XPATH)
        assert sum(one_total_sum_in_eur) == Decimal(
    ...
    def verify_sum_all_totals_for_types(self):
        all_totals_sum_in_eur = self.wait_until_all_displayed(xpath=self.all_pg_tables_const.TOTAL_ALL_TYPES_VALUES_XPATH)
        one_total_sum_in_eur = [Decimal(total_in_eur.text) for total_in_eur in all_totals_sum_in_eur]
        total_type_sum_in_eur = self.get_element_text(self.all_pg_tables_const.TOTAL_ALL_TYPES_SUM_XPATH)
        assert sum(one_total_sum_in_eur) == Decimal(
    ...
    def verify_all_sum_equals(self):
        currency_total_sum_in_eur = self.get_element_text(xpath=self.all_pg_tables_const.TOTAL_IN_EUR_ALL_XPATH)
        team_total_sum_in_eur = self.get_element_text(xpath=self.all_pg_tables_const.TOTAL_ALL_TEAMS_SUM_XPATH)
        type_total_sum_in_eur = self.get_element_text(xpath=self.all_pg_tables_const.TOTAL_ALL_TYPES_SUM_XPATH)
